...
- **Multi-issue mode.** To group *otherwise-unrelated* issues as one reviewable unit, add a `# ai-implement.yml` block (`feature_branch.mode: "multi-issue"`) to the **parent's description** → its branch becomes `ai-implement/multi-issue/<key>` instead of `ai-implement/feature/<key>`, **identical in every other respect**. Default (label / no block) is feature-node. Write examples as `# ai-implement.yml (example)` — a bare marker line is stripped from the issue's own spec. See `docs/feature-branch-grouping.md`.
