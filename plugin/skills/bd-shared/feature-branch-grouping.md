...
for other differences won't find any. The grouped issues are enumerated in the top-of-tree
roll-up PR body (§4, `Grouped issues:`), not in the branch name. The block must be a **fenced code block** (any ``` fence — the language
...
- **Top-of-tree PR** (no feature-node parent) → an open `feature → base` PR titled
  ...
  Its body enumerates a **`Grouped issues:`** list of the grouped child issues (**both modes**) — this
  compensates for the branch name not carrying the child keys, so a landing skill can confirm every child
  made it in. (If a roll-up PR ever lacks the line — older PR / hand-opened — verify child landing against
  the tracker's parent/child hierarchy instead.)
