...
2. Print exactly one line:
   ...
   - If `token.clientPath` is `https` (claude.ai connector session):
     > Orchestrator MCP token expired — reconnect the connector in [claude.ai](https://claude.ai) settings and retry.
   - If `token.clientPath` is `unknown`, empty or absent (e.g. a token that predates client attribution, or `get_session_identity` itself returned 401):
